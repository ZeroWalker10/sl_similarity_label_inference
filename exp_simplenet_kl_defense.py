...
skip_datasets = ['fashion-mnist', 'intel_classification', 'image-net',
                 'cifar10', 'fruits','hist_cancer_detection']
save_pathes = ['output/fashion-mnist-kl-defense-simplenet.pickle',
               'output/cifar10-kl-defense-simplenet.pickle',
               'output/dogs-vs-cats-kl-defense-simplenet.pickle',
               'output/hist-kl-defense-simplenet.pickle',
               'output/intel-kl-defense-simplenet.pickle',
               'output/fruits-kl-defense-simplenet.pickle',
               'output/imagenet-kl-defense-simplenet.pickle',]
...
for (n_epoch, lr, batch_size, data_path, dataset, composer, save_path, metric) in \
    ...
    for epsilon in epsilons:
        ...
        for i in range(2):
            ...
            # prepare sub models
            prev_model = nn.Sequential(*children[:split_pos+2])
            back_model = nn.Sequential(*children[split_pos+2:])
            # pdb.set_trace()
        ...
        # result = rm.load()
        print_log(['current performance:', layer_attacks], oriention=oriention)
